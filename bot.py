...
def _start_inv_init(tid):
    conn = get_db()
    items = conn.execute("SELECT * FROM nomenclature WHERE active=1 ORDER BY code").fetchall(); conn.close()
    rows = [[("📋 "+it["code"]+" — "+it["name"], f"inv:n:{it['id']}")] for it in items]
    kb = ik(*rows)
...
def _start_inv_adj(tid):
    conn = get_db()
    items = conn.execute("SELECT * FROM nomenclature WHERE active=1 ORDER BY code").fetchall(); conn.close()
    rows = [[("📋 "+it["code"]+" — "+it["name"], f"inv:adj:n:{it['id']}")] for it in items]
    kb = ik(*rows)
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("inv:adj:add:") or c.data.startswith("inv:adj:sub:") or c.data.startswith("inv:adj:set:"))
def inv_adj_type(call):
    parts = call.data.split(":"); action=parts[2]; nom_id=int(parts[3])
    user_data[call.from_user.id]["adj_type"] = action
    user_data[call.from_user.id]["nom_id"]   = nom_id
    labels = {"add":"прибавить","sub":"убрать","set":"установить значение"}
    user_states[call.from_user.id] = "inv:adj:qty"
    ans(call)
...
def _show_item_picker(tid):
    ...
    for i in range(0, len(items), 2):
        row = [(items[i]["code"]+" "+items[i]["name"][:20], f"ni:{items[i]['id']}")]
        if i+1 < len(items): row.append((items[i+1]["code"]+" "+items[i+1]["name"][:20], f"ni:{items[i+1]['id']}"))
        rows.append(row)
...
def _show_prod_picker(tid):
    ...
    rows = []
    for i in range(0, len(items), 2):
        row = [(items[i]["code"]+" "+items[i]["name"][:18], f"pd:n:{items[i]['id']}")]
        if i+1 < len(items): row.append((items[i+1]["code"]+" "+items[i+1]["name"][:18], f"pd:n:{items[i+1]['id']}"))
        rows.append(row)
...
def universal_text_handler(message):
    ...
    if state.startswith("ship:") and state.endswith(":date"):
        ...
        it = items[0]; rem = it["quantity"]-it["shipped_qty"]
        bot.send_message(tid,
            f"Позиция 1/{len(items)}: *{it['code']} {it['name']}*\nОстаток: {rem:,.1f} {it['unit']}\n\nВведите количество:",
