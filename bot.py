...
def handle_callback(call):
    ...
    if cd == "ord:list":
        ans(call)
        try:
            t,k = _orders_list_view(user["role"])
            try:
                bot.edit_message_text(t, call.message.chat.id, call.message.message_id, reply_markup=k, parse_mode="Markdown")
            except:
                bot.send_message(call.message.chat.id, t, reply_markup=k, parse_mode="Markdown")
        except Exception as e:
            bot.send_message(call.message.chat.id, f"❌ Ошибка реестра заказов: {e}")
        return
