...
def handle_callback(call):
    ...
    # Заказы
    if cd == "ord:list":
        ans(call); t,k=_orders_list_view(user["role"]); edit(call,t,k); return
...
def _orders_list_view(role):
    ...
    lines = ["<b>📦 Реестр заказов:</b>\n"]; kb_rows = []
    for o in orders:
        st   = ORDER_STATUS_LABELS.get(o["status"],o["status"])
        date = datetime.fromisoformat(o["created_at"]).strftime("%d.%m") if o["created_at"] else "—"
        due  = f" ⏰{o['desired_date']}" if o["desired_date"] else ""
        cp   = o["cp_name"] or "—"
        lines.append(f"• {o['number']} | {cp} | {st} | {date}{due}")
        kb_rows.append([(f"{o['number']} | {cp} | {st}", f"ord:v:{o['id']}")])
    kb_rows.append([("🔙 Назад","mn:sales")])
    return "\n".join(lines), ik(*kb_rows), "HTML"
