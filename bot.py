...
def init_db():
    ...
    add_col("users",          "daily_rate REAL DEFAULT 0")
    add_col("nomenclature",   "initial_stock REAL DEFAULT 0")
    add_col("nomenclature",   "cost_price REAL DEFAULT 0")
    add_col("nomenclature",   "sale_price REAL DEFAULT 0")
    add_col("order_items",    "shipped_qty REAL DEFAULT 0")
    add_col("orders",         "notes TEXT")
    add_col("shipments",      "notes TEXT")
    add_col("shipments",      "counterparty_id INTEGER")
    add_col("shipment_items", "order_item_id INTEGER")
    add_col("shipment_items", "nomenclature_id INTEGER")
    add_col("shipment_items", "sale_price REAL DEFAULT 0")
...
def is_work_time(): return 8 <= now().hour < 20
...
@bot.message_handler(commands=["clearstaff"])
def cmd_clearstaff(m):
    u = get_user(m.from_user.id)
    if not u or u["role"] != "superadmin":
        bot.send_message(m.from_user.id,"Только Главный Админ."); return
    conn = get_db()
    count = conn.execute("SELECT COUNT(*) FROM users WHERE telegram_id != ?", (m.from_user.id,)).fetchone()[0]
    conn.execute("DELETE FROM users WHERE telegram_id != ?", (m.from_user.id,))
    conn.commit(); conn.close()
    bot.send_message(m.from_user.id, f"Удалено {count} сотрудников. Остался только ты.")
...
def btn_checkout(m):
    ...
    now = now(); hrs = (now - datetime.fromisoformat(active["check_in"])).total_seconds()/3600
...
def btn_stats(m):
    u = get_user(m.from_user.id)
    if not u or u["role"] != "worker": return
    now = now(); ms = now.replace(day=1,hour=0,minute=0,second=0,microsecond=0)
    conn = get_db()
...
def handle_cb(call):
    ...
    if cd == "shift:now":
        ...
        for r in rows_db:
            dt=datetime.fromisoformat(r["check_in"]); hrs=(now()-dt).total_seconds()/3600
    ...
    # Финансы
    if cd == "exp:list":
        now=now(); ms=now.replace(day=1).strftime("%d.%m.%Y")
        conn=get_db()
    ...
    if cd == "sal:report":
        now=now(); ms=now.replace(day=1,hour=0,minute=0,second=0,microsecond=0); month_str=now.strftime("%Y-%m")
...
def handle_text(message):
    ...
    # Прямая отгрузка
    if state == "ds:date":
        user_data[tid]["ship_date"]=now().strftime("%d.%m.%Y") if text=="-" else text
    ...
    if state == "pd:qty":
        ...
        d=user_data[tid]; nom=d.pop("current_nom"); today=d["date"]; now=now()
        conn=get_db()
    ...
    if state.startswith("exp:fixed:") and state.endswith(":from"):
        etype=state.split(":")[2]; eff=now().strftime("%d.%m.%Y") if text=="-" else text; d=user_data[tid]
    ...
    if state == "exp:var:desc":
        d=user_data[tid]; desc=None if text=="-" else text; today=now().strftime("%d.%m.%Y")
    ...
    if state == "sal:bonus:desc":
        d=user_data[tid]; desc=None if text=="-" else text; month_str=now().strftime("%Y-%m")
