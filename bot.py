...
def handle_callback(call):
    ...
    if cd.startswith("usr:role:"):
        ...
        cancel_state(call.from_user.id); return
...
def _nom_detail(nom_id, role):
    ...
    if role in ("admin","superadmin"):
        ...
        kb_rows.append([("💰 Изменить себестоимость",f"nm:price:{nom_id}:cost")])
        kb_rows.append([("💵 Изменить прайс",f"nm:price:{nom_id}:sale")])
        kb_rows.append([("📜 История цен",f"nm:ph:{nom_id}")])
...
def _cp_detail(cp_id, role):
    ...
    if role in ("admin","superadmin"):
        ...
        kb_rows.append([("✏️ Примечание",f"cp:edit:{cp_id}:notes")])
...
def _fixed_expenses_view(role):
    ...
    for etype, elabel in FIXED_EXPENSE_TYPES:
        ...
        if role in FINANCE_ROLES:
            ...
            rows_kb.append([(f"📜 История {elabel}", f"exp:fixed:hist:{etype}")])
    conn.close()
...
def _start_var_expense(tid):
    rows = [
        [("🏠 Аренда","exp:var:cat:Аренда"),("💡 Коммунальные","exp:var:cat:Коммунальные")],
        [("🗑 Мусор","exp:var:cat:Мусор"),("👷 Зарплата","exp:var:cat:Зарплата")],
...
def universal_text(message):
    ...
    if state.startswith("nm:price:"):
        ...
        bot.send_message(tid,f"✅ {label} обновлена: *{price:,.2f} ₽*", parse_mode="Markdown"); return
    ...
    if state.startswith("exp:fixed:") and state.endswith(":from"):
        ...
        cancel_state(tid)
        label=dict(FIXED_EXPENSE_TYPES).get(etype,etype)
        bot.send_message(tid,f"✅ *{label}*: {amount:,.2f} ₽ с {eff_from}", parse_mode="Markdown"); return
    ...
    if state=="exp:var:desc":
        ...
        conn.commit(); conn.close()
        cancel_state(tid)
        bot.send_message(tid,f"✅ Расход добавлен!\n{d['category']}: *{d['amount']:,.2f} ₽*", parse_mode="Markdown"); return
