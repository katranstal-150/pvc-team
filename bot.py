...
def mgr_rk():
    kb = ReplyKeyboardMarkup(resize_keyboard=True)
    kb.row(KeyboardButton("Заказы"), KeyboardButton("Производство"))
    kb.row(KeyboardButton("Отгрузки"), KeyboardButton("Склад"))
    kb.row(KeyboardButton("Финансы"), KeyboardButton("Номенклатура"))
    kb.row(KeyboardButton("Контрагенты"), KeyboardButton("Управление"))
    return kb
...
def btn_orders(m):
    ...
    send_orders_list(m.from_user.id, u["role"])

@bot.message_handler(func=lambda m: m.text == "Производство")
def btn_prod(m):
    u = get_user(m.from_user.id)
    if not u or u["role"] not in ("manager","admin","superadmin"): return
    kb = ik([("Производство за день","pd:start")],[("Подтвердить отгрузку","cs:list")],[("Отчет по сотрудникам","rp:staff")])
    bot.send_message(m.from_user.id, "Производство:", reply_markup=kb)
